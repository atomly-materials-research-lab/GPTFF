...
def validate_required_labels(batch, config: TrainingConfig) -> None:
    if not loss_weight_active(config.loss.energy_loss_weight):
        raise ValueError("energy_loss_weight must be positive.")
    if not loss_weight_active(config.loss.force_loss_weight):
    ...
    if loss_weight_active(config.loss.stress_loss_weight) and batch.stress is None:
...
def compute_batch_loss(
    ...
    *,
    ...
    compute_stress = loss_weight_active(config.loss.stress_loss_weight)
    ...
    loss = loss + config.loss.energy_loss_weight * energy_loss
    ...
    loss = loss + config.loss.force_loss_weight * force_loss
    ...
    if compute_stress:
        ...
        loss = loss + config.loss.stress_loss_weight * stress_loss
        stress_mae = mae(
