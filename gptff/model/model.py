...
class TransformerBlock(nn.Module):
    def __init__(self, atom_fea_len):
        ...
        # 使用自定义的TransformerEncoderLayer而不是nn.TransformerEncoderLayer
        # 避免PyTorch的efficient attention问题
        self.encoder = TransformerEncoderLayer(atom_fea_len, 2, 256)

    def forward(self, atom_fea, masks):
        # 处理padding mask
        if masks is not None:
            # 将padding mask转换为attention mask
            # padding mask: True表示padding位置，False表示有效位置
            # attention mask: True表示要mask的位置，False表示有效位置
            attention_mask = masks.unsqueeze(1).unsqueeze(2)  # [B, 1, 1, N]
            attention_mask = attention_mask.expand(-1, -1, atom_fea.size(1), -1)  # [B, 1, N, N]
        else:
            attention_mask = None
            
        # 应用mask到输入
        if attention_mask is not None:
            # 将padding位置的feature设为0
            atom_fea = atom_fea * (~masks).unsqueeze(-1).float()
        
        out = self.encoder(atom_fea)
